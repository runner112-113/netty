...
public class LengthFieldBasedFrameDecoder extends ByteToMessageDecoder {
     ...
     * 计算获取需要丢弃的字节数之后，调用ByteBuf的skipBytes方法跳过需要忽略的字节长度，然后bytesToDiscard减去已经忽略的字节长度。
    ...
    /**
     * 丢弃的策略如下：
     * frameLength减去ByteBuf的可读字节数就是需要丢弃的字节长度，如果需要丢弃的字节数discard小于缓冲区可读的字节数，则直接丢弃整包消息。
     * 如果需要丢弃的字节数大于当前的可读字节数，说明即便将当前所有可读的字节数全部丢弃，也无法完成任务，则设置discardingTooLongFrame 标识为true，下次解码的时候继续丢弃。
     * 丢弃操作完成之后，调用faillfNecessary方法根据实际情况抛出异常
     */
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
        ...
        if (frameLengthInt == -1) { // new frame
            ...
            // 对当前缓冲区的可读字节数和长度偏移量进行对比，如果小于长度偏移量，则说明当前缓冲区的数据报不够，需要返回空，由I/0线程继续读取后续的数据报
            ...
            // 如果长度小于O，说明报文非法，跳过lengthFieldEndOffset个字节，抛出CorruptedFrameException异常。
            if (frameLength < 0) {
        ...
        /**
         * 根据消息的实际长度分配一个新的ByteBuf对象，将需要解码的ByteBuf可写缓冲区复制到新创建的ByteBuf 中并返回，
         * 返回之后更新原解码缓冲区ByteBuf为原读索引+消息报文的实际长度（actuaIFrameLength）。
         */
        // extract frame
        int readerIndex = in.readerIndex();
        int actualFrameLength = frameLengthInt - initialBytesToStrip;
        ByteBuf frame = extractFrame(ctx, in, readerIndex, actualFrameLength);
        // 更新原解码缓冲区ByteBuf为原读索引+消息报文的实际长度（actuaIFrameLength）
